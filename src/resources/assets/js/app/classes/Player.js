...
, function ($, _, Abstract, F) {
  ...
  const Player = class Player extends Abstract {
    ...
    Populate () {
      ...
      self.m._data.instance        = $(self.m._config.selector)[0];
      self.m._data.instance.volume = self.m._config.volume;

      return Promise.resolve().then(function () {
        return new Promise(function (resolve, reject) {

          $(self.m._config.tracks.container)
            .find(self.m._config.tracks.selector)
            .each( function (idx, el) {
              self.m._data.tracks.list.push( $(el).attr('href') );
            });
          $('#files-count').text(self.m._data.tracks.list.length);

          // self.m._data.instance.addEventListener('play',  self.setButtonPlay.bind(this),  false);
          // self.m._data.instance.addEventListener('pause', self.setButtonPause.bind(this), false);
          // self.m._data.instance.addEventListener('ended', self.stepForward.bind(this),    false);
          //
          // self.m._data.instance.addEventListener('canplay',    self.initCounters,  false);
          // self.m._data.instance.addEventListener('timeupdate', self.checkTimeLeft, false);
  ...
  //     btnPlay.removeClass(state)
  //       .children('i')
  //       .removeClass(self._config.states[state]['icon']);
  //     domProgress.removeClass(self._config.states[state]['progress']);
  //   }
  //   return self;
  // };
  //
